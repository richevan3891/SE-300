...
public class Main implements ActionListener {
	...
	public JMenuBar menuBar() {
		...
		menuItem = new JMenuItem("Canceled Routes");
		menu.add(menuItem);
		menuItem.addActionListener(new RouteClosedListener());
	...
	class RouteClosedListener implements ActionListener{
		public void actionPerformed(ActionEvent event) {
			if (systemLocked == false) {
				String[][] routesList = Main.convertArrayListTo2DArray(routesInfo, 1);
				//RouteClosed closedRoutes = new RouteClosed(routesList);
				Object[][] routesClosed = RouteClosed.searchClosedRoute(routesList);
				Main.updateResultsTable(routesClosed);
			}
			else {
				String historyText = "You must open a valid file to be able to perform this action.\n";
				historyArea.append(historyText);
