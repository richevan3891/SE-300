...
public class Route_searchroute_dialogue extends JDialog {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4795740566809937836L;
	...
	public Route_searchroute_dialogue(String[] Airports, String[] Routenums) {
		...
		Carrierchoice = new JComboBox<Object>(Carriers);
		DepAPchoice = new JComboBox<Object>(Airports);
		ArrAPchoice = new JComboBox<Object>(Airports);
	...
	static class searchRouteListener implements ActionListener{
		public void actionPerformed(ActionEvent event) {
			...
			if (FindRouteNum.routeNumSelected == true) {
				...
				depTimeHrString = TimeSelectBoxPanel.Hourlist[depTimeHrInt];

				depTimeMinInt = Deptimechoice.Minutechoice.getSelectedIndex();
				depTimeMinString = TimeSelectBoxPanel.Minutelist[depTimeMinInt];
				...
				arrTimeHrString = TimeSelectBoxPanel.Hourlist[arrTimeHrInt];

				arrTimeMinInt = Arrtimechoice.Minutechoice.getSelectedIndex();
				arrTimeMinString = TimeSelectBoxPanel.Minutelist[arrTimeMinInt];
	...
	public static Object[][] matchRouteNum(int routeNumInt, String routeNumString, String[] routeNums, String[][] routes) {
		...
		if (pathFound[7].equals("true")) {
			for (int i = 0; i < pathFound.length-1; i++) {
				routesDataStr[0][i] = pathFound[i];
			}
			historyText = "Your search returned one result that is open for flight, which is displayed in the search results table.";
		}
		else {
			for (int i = 0; i < pathFound.length-1; i++) {
				routesDataStr[0][i] = null;
			}
			historyText = "Route " + routeNumString + " is serviced at an airport that is not currently open.\nOnce the airport servicing" +
					" this route is reopened, this route will be searchable again.";
		}
	...
	public static Object[][] matchDepAP(String depAPString, String[][] routes) {
		...
		for (int i = 0; i < routes.length; i++) {
			if (routes[i][2].equals((depAPString)) && routes[i][7].equals("true")) {
				count = count + 1;
			}
		}
		...
		if (depCount == 1) {
			historyText = "There is " + count + " open route that will leave from the " + depAPString + " airport.";
		}
		else {
			historyText = "There are " + count + " open routes that will leave from the " + depAPString + " airport.";
		}
		Main.historyArea.append(historyText + "\n");
	...
	public static Object[][] matchArrAP(String arrAPString, String[][] routes) {
		...
		for (int i = 0; i < routes.length; i++) {
			if (routes[i][4].equals((arrAPString)) && routes[i][7].equals("true")) {
				count = count + 1;
			}
		}
		...
		if (arrCount == 1) {
			historyText = "There is one " + count + " open route that will arrive at the " + arrAPString + " airport.";
		}
		else {
			historyText = "There are " + count + " open routes that will arrive at the " + arrAPString + " airport.";
		}
		Main.historyArea.append(historyText + "\n");
	...
	static Object[][] depTimeMethod(String depTimeHrString, String depTimeMinString, String[][] routes) {
		...
		Object[][] routesData = new Object[0][7];
		for (int i = 0; i < routes.length; i++) {
			if (routes[i][3].equals(depTimeHrString + depTimeMinString) && routes[i][7].equals("true")) {
				count = count + 1;
			}
		}
		...
		if (count == 1) {
			historyText = "There is " + count + " open route listed that will depart at " + depTimeHrString + ":" + depTimeMinString + " from" +
					" the " + depTimeString[0][2] + " airport.";
		}
		else {
			historyText = "There are " + count + " open routes listed that will depart at " + depTimeHrString + ":" + depTimeMinString + ".";
		}
		Main.historyArea.append(historyText + "\n");
	...
	public static Object[][] arrTimeMethod(String arrTimeHrString, String arrTimeMinString, String[][] routes) {
		...
		Object[][] routesData = new Object[0][7];
		for (int i = 0; i < routes.length; i++) {
			if (routes[i][5].equals(arrTimeHrString + arrTimeMinString) && routes[i][7].equals("true")) {
				count = count + 1;
			}
		}
		...
		if (count == 1) {
			historyText = "There is " + count + " open route listed that will arrive at " + arrTimeHrString + ":" + arrTimeMinString + " from" +
					" the " + arrTimeString[0][2];
		}
		else {
			historyText = "There are " + count + " open routes listed that will arrive at " + arrTimeHrString + ":" + arrTimeMinString + ".";
		}
		Main.historyArea.append(historyText + "\n");
	...
	public static Object[][] carrierMethod(String carrierString, String[][] routes) {
		...
		Object[][] routesData = new Object[0][7];
		for (int i = 0; i < routes.length; i++) {
			if (routes[i][1].equals(carrierString) && routes[i][7].equals("true")) {
				count = count + 1;
			}
		}
		...
		if (count == 1) {
			historyText = "There is " + count + " open route listed that will be flying with " + carrierString + " today.";
		}
		else {
			historyText = "There are " + count + " open routes listed that will be flying with " + carrierString + " today.";
		}
		Main.historyArea.append(historyText + "\n");
