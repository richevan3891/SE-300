...
public class Main implements ActionListener {
	...
	public JMenuBar menuBar() {
		...
		menuItem = new JMenuItem("Open/Close");
		menu.add(menuItem);
	...
	class AirAllListener implements ActionListener{
		public void actionPerformed(ActionEvent event) {
			...
			String[] allAirports = convertAirportArrayListToArray(input.airportArrayList(input.airportsToken));
			int count = allAirports.length;
			int airCount = 0;
			for (int i = 0; i < allAirports.length; i++) {
				if (airCount == 0) {
					historyText = historyText + " " + allAirports[i];
				}
