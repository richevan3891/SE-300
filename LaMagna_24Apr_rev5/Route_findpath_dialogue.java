...
public class Route_findpath_dialogue extends JDialog {
	...
	static private JLabel Fastlabel = new JLabel("Fastest (Air Time)", SwingConstants.CENTER);
	static private JLabel Perccarrierlabel = new JLabel("% Carrier", SwingConstants.CENTER);
	...
	static JRadioButton Checkperccarrier = new JRadioButton();
	...
	public Route_findpath_dialogue(String[] Airports) {
		...
		radioGroup.add(Checkperccarrier);
		...
		Radiopanel.add(Checkperccarrier);
		Radiopanel.add(Perccarrierlabel);
	...
	public int Path_Properties() {
		...
		for (int i = 0; i < resultsInfo.size(); i++) {
			...
			for (int j = 0; j < resultsInfo.get(i).length; j++) {
				...
				for (int k = 0; k < Main.routesInfo.size(); k++) {
					if (Main.routesInfo.get(k)[0].equalsIgnoreCase(routenum)) {
						...
						// Increase carrierCount by one if the carrier matches the preference
						if (Main.routesInfo.get(k)[1].equalsIgnoreCase((String) Carrierchoice.getSelectedItem())) {
							carrierCount = carrierCount + 1;
						}
						
						// If there is a route after this one, add layover time to total path time
						if (j + 1 < resultsInfo.get(i).length) {
							// grab the next routenumber
							String nextroutenum = resultsInfo.get(i)[j+1];
							
							// go through the main table until the next route in path is found, grab its deptime, and add layover to pathtime
							for (int l = 0; l < Main.routesInfo.size(); l++) {
								if (Main.routesInfo.get(l)[0].equalsIgnoreCase(nextroutenum)) {
									
									layoverTime = layoverTime + (routetime2doub(Main.routesInfo.get(l)[3]) - routetime2doub(Main.routesInfo.get(k)[5]));
									
								}
							}
							
						}// End adding layover time to path time
			...
			percentCarrier = carrierCount/resultsInfo.get(i).length;
			
			double[] path_props = {pathTime, pathCost, layoverTime, percentCarrier};
	...
	public void sortResults(int sortOption) {
		...
		if (sortOption == 3) {
			...
			for (int i = 0; i < resultsInfo.size(); i++) {
				...
				// Rearrange the Arraylists
				for (int j = i; j < resultsProps.size(); j++) {
					if (resultsProps.get(j)[3] > tempProp[3]) {
	...
	public void updatePathTable(int necessary_array_size) {
		...
		for (int i = 0; i < resultsInfo.size(); i++) {
			...
			PathTable[current_index][1] = "Air Time";
			PathTable[current_index][2] = doubtime2string(resultsProps.get(i)[0]);
			PathTable[current_index][3] = "Total Time";
			PathTable[current_index][4] = doubtime2string(resultsProps.get(i)[0] + resultsProps.get(i)[2]);
			PathTable[current_index][5] = Integer.toString((int) (resultsProps.get(i)[3]*100)) + "% " + (String) Carrierchoice.getSelectedItem();
	...
	class findPathListener implements ActionListener{
		public void actionPerformed(ActionEvent event) {			
			...
			if (Checkperccarrier.isSelected()) {
				sort = 3;
			}
			...
			if (DepAP_option.equalsIgnoreCase(ArrAP_option)) {
				APerror = true;
				Main.historyArea.append("Departure and Destination airports cannot be the same\n");
			}
			if (Deptime_option >= Finaldesttime_option) {
				timeerror = true;
				Main.historyArea.append("Arrival time cannot be before the departure time\n");
			}
			if (!APerror && !timeerror) {
				...
				Main.historyArea.append("There are " + number_of_results + " paths found from " + DepAP_option + " to " + ArrAP_option + ".\n");
