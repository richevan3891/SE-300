...
public class Route_addroute_dialogue extends JDialog {
	...
	class addRouteListener implements ActionListener{
		public void actionPerformed(ActionEvent event) {
			...
			try {
				routeNumInt = Integer.parseInt(Routenumtxt.getText().trim());
			} catch (Exception e) {
				routeError = true;
			}
			...
			if ((arrHourInt - depHourInt) < 1 && (arrMin - depMin < 30)) {
				timeError = true;
			}
			...
			if ((routeError == false) && (pathError == false) && (priceError == false) && (routeExists == false) && (timeError == false)) {
				String[] dummyRoute = {routeNumString, carrierString, depAirStr, depString, arrAirStr, arrString, totalPrice};	 //TEMPORARY DUMMY ROUTE INFO
				...
				historyText = "Route number " + routeNumString + " already exists. Enter a different route number and then " +
