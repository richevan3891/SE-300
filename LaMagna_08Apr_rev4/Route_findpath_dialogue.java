...
public class Route_findpath_dialogue extends JDialog {
	...
	public void Find_Path(String DepAP, String ArrAP, double Deptime, double Finaldesttime, String Carrier, String[] PrevAP, String[] PrevRoutes, int iteration) {
		...
		for (int i = 0; i < Main.routesInfo.size(); i++) {
			...
			String [] thisRouteDepAP = thisRoute[2].split(" ");
			if (!(thisRouteDepAP[1].equalsIgnoreCase(DepAP))) {
				rightstartpoint = false;
			}
			
			
			// Check to see if the destination has already been visited
			String [] thisRouteArrAP = thisRoute[4].split(" ");
			for (int k = 0; k < PrevAP.length; k++ ) {
				if (PrevAP[k] == null) {
					break;
				}
				
				else if (PrevAP[k].equalsIgnoreCase(thisRouteArrAP[1])) {
			...
			String [] thisRouteCarrier = thisRoute[1].split(" ");
			if ( !( thisRouteCarrier[1].equalsIgnoreCase(Carrier) || Carrier.equalsIgnoreCase("None") ) ) {
				rightCarrier = false;
			}
		...
		for (int i = 0; i < routes_to_evaluate.size(); i++) {
			...
			String [] evalRouteArrAP = evalRoute[4].split(" ");
			if (evalRouteArrAP[1].equalsIgnoreCase(ArrAP) && routetime2doub(evalRoute[5]) <= Finaldesttime) {
				
				// Update the progression of routes to this solution
				String [] evalRoutenum = evalRoute[0].split(" ");
				PrevRoutes[iteration - 1] = evalRoutenum[0];
				...
				Find_Path(evalRouteArrAP[1].toString(), ArrAP, routetime2doub(evalRoute[5]), Finaldesttime, Carrier, PrevAP, PrevRoutes, iteration);
	...
	public double routetime2doub(String routetime) {
		...
		String[] timestring = routetime.split(" ");
		int tempvalue = Integer.parseInt(timestring[1]);
	...
	// Method for taking in a time in hrs and outputing the strin xhrs xmin
