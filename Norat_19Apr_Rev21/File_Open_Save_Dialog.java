	...
	File_Open_Save_Dialog(int openOrSave) throws IOException {
		...
		//If user selects save
			...
			if ( returnVal == JFileChooser.APPROVE_OPTION ) {		
				...
				fileSaveText = fileChooser.getSelectedFile().getAbsolutePath();
				//System.out.println(fileSaveText);
				//add .txt to file name if it does not already have it
				if (fileName.endsWith(".txt") == false) {
					fileName = fileName.concat(".txt");
				}
				if (fileSaveText.endsWith(".txt") == false) {
					fileSaveText = fileSaveText.concat(".txt");

				}
				saveFile(Main.airportsInfo, Main.routesInfo, Main.closuresInfo, fileSaveText);
			}
		}
	}
